        ...
        <form action="{{ route('admin.coupons.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="code" required value="{{ old('code') }}" class="w-full px-6 py-4 rounded-xl border border-gray-100 bg-gray-50/50 outline-none focus:border-[#2D4A1E] font-bold">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-6 py-4 rounded-xl border border-gray-100 bg-gray-50/50 outline-none focus:border-[#2D4A1E] font-bold">
                        <option value="flat">Flat Discount (Tk)</option>
                ...
                <div>
                    ...
                    <input type="number" name="value" step="0.01" required value="{{ old('value') }}" class="w-full px-6 py-4 rounded-xl border border-gray-100 bg-gray-50/50 outline-none focus:border-[#2D4A1E] font-bold">
                </div>

                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Min Order Amount (Tk)</label>
                    <input type="number" name="min_order" value="{{ old('min_order', 0) }}" class="w-full px-6 py-4 rounded-xl border border-gray-100 bg-gray-50/50 outline-none focus:border-[#2D4A1E] font-bold">
                </div>

                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Max Uses (Empty for Unlimited)</label>
                    <input type="number" name="max_uses" value="{{ old('max_uses') }}" class="w-full px-6 py-4 rounded-xl border border-gray-100 bg-gray-50/50 outline-none focus:border-[#2D4A1E] font-bold">
                </div>

                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Expiry Date</label>
                    <input type="date" name="expires_at" value="{{ old('expires_at') }}" class="w-full px-6 py-4 rounded-xl border border-gray-100 bg-gray-50/50 outline-none focus:border-[#2D4A1E] font-bold">
                </div>
            </div>
            ...
            <div class="mt-12 pt-8 border-t border-gray-50">
                <button type="submit" class="w-full bg-[#2D4A1E] text-white py-4 rounded-xl font-bold hover:bg-[#1f3513] transition shadow-lg">Create Coupon</button>
            </div>
        </form>
    </div>
</div>
